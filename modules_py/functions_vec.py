...
eq_pnn_corr = np.vectorize(f.eq_pnn_corr, excluded=['pf', 'part'])
...
eq_pnn_corr_ff_pnd_corr_ff = np.vectorize(f.eq_pnn_corr_ff_pnd_corr_ff, excluded=['pf', 'part'])

d0 = np.vectorize(f.d0, excluded=['width'])
d_pnn = np.vectorize(f.d_pnn, excluded=['pf', 'width', 'pi_pnn_part'])
d_pnd = np.vectorize(f.d_pnd, excluded=['pf', 'width', 'pi_pnd_part'])
d = np.vectorize(f.d, excluded=['pf', 'width', 'part' ])
d_pnn_corr = np.vectorize(f.d_pnn_corr, excluded=['pf', 'width', 'part'])
d_pnn_corr_pnd = np.vectorize(f.d_pnn_corr_pnd, excluded=['pf', 'width', 'part'])
d_pnn_corr_pnd_corr = np.vectorize(f.d_pnn_corr_pnd_corr, excluded=['pf', 'width', 'part'])
d_pnn_corr_ff_pnd_corr_ff = np.vectorize(f.d_pnn_corr_ff_pnd_corr_ff, excluded=['pf', 'width', 'part'])
